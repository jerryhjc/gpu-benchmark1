...
//#define NUM_TRANSACTIONS 50000
// #define NUM_TRANSACTIONS 384
...
__global__ void interac( account* accounts, transaction* transactions, int numTransactions)
{
	int id = THREAD_ID; 
	for(int index = id; index < numTransactions; index += TOTAL_THREADS)
	{
      ...
      /*
		...
		{
			if(atomicCAS(&lock1->lock, 0, 1))
			{
				continue;
			}
			
			if(atomicCAS(&lock2->lock, 0, 1))
			{
...
#elif defined(NO_SYNC)

		src->balance -= action->amount;
		dest->balance += action->amount;
		
#else
	#error You must specify a synchronization method...
#endif
...
int main(int argc, char** argv)
{
    ...
    // execute the kernel
    interac<<< grid_size, block_size >>>(device_accounts, device_transactions, NUM_TRANSACTIONS);
