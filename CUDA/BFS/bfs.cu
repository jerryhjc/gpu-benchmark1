...
//    CUT_DEVICE_INIT();
    ...
    bool32 *h_graph_mask = (bool32*) malloc(sizeof(bool32)*no_of_nodes);
    bool32 *h_graph_visited = (bool32*) malloc(sizeof(bool32)*no_of_nodes);
    ...
    bool32* d_graph_mask;
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_graph_mask, sizeof(bool32)*no_of_nodes) );
    CUDA_SAFE_CALL( cudaMemcpy( d_graph_mask, h_graph_mask, sizeof(bool32)*no_of_nodes, cudaMemcpyHostToDevice) );
    
    //Copy the Visited nodes array to device memory
    bool32* d_graph_visited;
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_graph_visited, sizeof(bool32)*no_of_nodes) );
    CUDA_SAFE_CALL( cudaMemcpy( d_graph_visited, h_graph_visited, sizeof(bool32)*no_of_nodes, cudaMemcpyHostToDevice) );
    ...
    bool32 *d_over;
    CUDA_SAFE_CALL( cudaMalloc( (void**) &d_over, sizeof(bool32)));
	...
	stop=false;
	CUDA_SAFE_CALL( cudaMemcpy( d_over, &stop, sizeof(bool32), cudaMemcpyHostToDevice) );
    	CUT_SAFE_CALL( cutStartTimer( timer));
	...
	CUDA_SAFE_CALL( cudaMemcpy( &stop, d_over, sizeof(bool32), cudaMemcpyDeviceToHost) );
    k++;
	}
    while(stop);
